...
export function getNowDateFormattedChineseDate(): string {
  ...
  const date = cnDayNums[now.date() - 1];
...
/**
 * @description: 获取本月第一天的上个周日距离本月第一天相差的天数
 * @return {number} 本月第一天是周三到上月最后一天的上周日 => 3天
 */
export function getDaysFromFirstDayToLastSunday(): number {
  const firstDay = dayjs().startOf("month"); // 当前月份第一天
  const lastSunday = firstDay.subtract(1, "day").startOf("week"); // 上个周日
  const daysDiff = firstDay.diff(lastSunday, "day"); // 计算相差天数
  return daysDiff + 1; // 返回相差天数
}
